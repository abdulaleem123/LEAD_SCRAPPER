...
import { defaults, freePlatforms, settingsSchema, type Settings, type Opportunity, type Job, type Scan } from "./types";
import type { SocialCandidate } from './social-index';
...
export function db() {
  ...
  connection.exec(`
    ...
    CREATE TABLE IF NOT EXISTS social_candidates(id TEXT PRIMARY KEY, json TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'new', discoveredAt TEXT NOT NULL);
  `);
  return connection;
}
export function freeSettings(s:Settings):Settings {
  const selected=s.platforms.filter(p=>freePlatforms.some(f=>f===p));
  return {...s,platforms:selected.length?selected:[...freePlatforms],aiReview:false};
}
export function settings(): Settings { const r = db().prepare("SELECT json FROM opportunity_settings WHERE id=1").get() as {json: string} | undefined; return freeSettings(r ? settingsSchema.parse(JSON.parse(r.json)) : structuredClone(defaults)); }
export function saveSettings(s: Settings) { db().prepare("INSERT INTO opportunity_settings(id,json) VALUES(1,?) ON CONFLICT(id) DO UPDATE SET json=excluded.json").run(JSON.stringify(freeSettings(s))); }
...
export function insertCandidate(c:SocialCandidate){return db().prepare('INSERT OR IGNORE INTO social_candidates(id,json,discoveredAt) VALUES(?,?,?)').run(c.id,JSON.stringify(c),c.discoveredAt).changes>0;}
export function listCandidates():SocialCandidate[]{return (db().prepare('SELECT json,status FROM social_candidates ORDER BY discoveredAt DESC LIMIT 1000').all() as {json:string;status:SocialCandidate['status']}[]).map(r=>({...JSON.parse(r.json),status:r.status}));}
export function editCandidate(id:string,status:SocialCandidate['status']){return db().prepare('UPDATE social_candidates SET status=? WHERE id=?').run(status,id).changes>0;}
