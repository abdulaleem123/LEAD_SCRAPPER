...
export const coverage: Record<Platform,string> = {
  hackernews: 'Public Hacker News posts and comments',
...
export function sourceError(e: unknown) {
  ...
  if (/token|unauthor|401/i.test(message)) return "Connection was rejected. Check the lead-engine token in the app environment file.";
  if (/402|credit|payment|rent|subscription|paid plan|charge|insufficient/i.test(message)) return "This source needs available lead-engine credit or a source subscription.";
